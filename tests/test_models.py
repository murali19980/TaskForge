...
def test_openrouter_model_validation():
    ...
    # Valid free suffix should pass
    s1 = Settings(OPENROUTER_MODEL="google/gemini-2.5-flash:free", OPENROUTER_API_KEY="test")
    assert s1.OPENROUTER_MODEL == "google/gemini-2.5-flash:free"

    # Valid free router should pass
    s2 = Settings(OPENROUTER_MODEL="openrouter/free", OPENROUTER_API_KEY="test")
    assert s2.OPENROUTER_MODEL == "openrouter/free"

    # Invalid paid model should raise ValidationError
    with pytest.raises(ValidationError) as exc_info:
